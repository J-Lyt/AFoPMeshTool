    ...
    class Mesh:
        class LOD:
            def __init__(self, parent_mesh,index):
                ...
                self.extra_vert_offset = 0   # file offset of extra vertex buffer (lod_info_type=0x0100)
                self.extra_vert_stride = 48  # bytes per vertex in the extra buffer
        ...
        def __init__(self,parent_sk_mesh, index=0):
            ...
            self.lod_info_type = 0  # stored after pre-LOD parse; needed by LOD.parse()
        ...
        def parse(self, f):
            ...
            f.seek(1, 1)
            # v19: x_count widened to uint16; last byte of last x_data entry is a skip factor
            # for an additional mystery section of (factor × 4) bytes after an extra unk byte.
            if version == 19:
                x_count = br.uint16(f)
                f.seek(1, 1)  # unk after x_count
                if x_count > 0:
                    f.seek(4 * (x_count - 1), 1)  # skip all but last x_data entry
                    f.seek(3, 1)                   # skip first 3 bytes of last entry
                    mystery_skip_factor = br.uint8(f)  # last byte of last entry = skip factor
                else:
                    mystery_skip_factor = 0
                f.seek(1, 1)                       # extra unk byte after x_data
                f.seek(mystery_skip_factor * 4, 1) # mystery section
            else:
                x_count = br.uint8(f)
                f.seek(1, 1)
                f.seek(4 * x_count, 1)
...
class BlenderMeshImporter:
    ...
    def import_mesh(file, skeletal_mesh:SkeletalMeshAsset, mesh:SkeletalMeshAsset.Mesh, lod_index = 0):
        ...
        # Stamp original MMB vertex index so order survives Separate/Loose Parts etc.
...
class BlenderMeshExporter:
    ...
    def write_vertices(file, mesh:SkeletalMeshAsset.Mesh, lod_index=0):
        ...
        if obj:
            ...
            # Build blender_to_mmb remapping from mmb_vertex_order attribute.
            # Survives Mesh > Separate > By Loose Parts and similar reordering ops.
    ...
    def execute(self, context):
        ...
        for m in asset.meshes:
            for li, lod in enumerate(m.lods):
                ...
                tri_obj = BME.find_object_by_name(lod_obj_name)
                if tri_obj:
                    BME._triangulate_object(tri_obj, compute_normals=SWOMT.compute_normals_on_export)
        ...
        # Apply header patches once after all LODs are written
        for mesh in asset.meshes:
            BME._apply_header_patches(mod_file, mesh, asset, operator=self)
